...
def extract_state_dict(data: Dict, state_space: Union[str, List[str]]) -> Dict:
    ...
    # XR-1 RoboCasa rollouts already store the selected 16d RoboCasa state.
    # Official human datasets may still store the original 25d state and need
    # the standard index selection above.  Support both representations.
    if raw_state.shape[-1] == len(all_state_ids):
        state_dict["state"] = raw_state
    else:
        state_dict["state"] = raw_state[..., all_state_ids]
