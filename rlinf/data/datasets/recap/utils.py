...
def load_task_descriptions(dataset_path: str | Path) -> dict[int, str]:
    ...
    # LeRobot v3 stores task tables in sharded parquet files under meta/tasks.
    # Keep this helper compatible with both the legacy flat file and v3 layout.
    task_shards = sorted((meta / "tasks").glob("**/*.parquet"))
    if task_shards:
        import pandas as pd

        frames = [pd.read_parquet(path) for path in task_shards]
