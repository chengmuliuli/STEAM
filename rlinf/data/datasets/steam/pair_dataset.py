...
def _install_pyav_video_decoder() -> None:
    """Use PyAV when torchcodec cannot load the server's FFmpeg libraries."""
    try:
        import av
        import torch
    ...
    if getattr(lerobot_dataset_module, "_rlinf_pyav_decoder_installed", False):
        return
    ...
    def decode_video_frames(video_path, timestamps, tolerance_s, backend=None):
        ...
        for timestamp in timestamps:
            ...
            if best is None:
                raise RuntimeError(
                    f"No frame decoded from {video_path} at {target}"
                )
            output.append(torch.from_numpy(best.to_ndarray(format="rgb24")))
        return torch.stack(output)

    lerobot_dataset_module.decode_video_frames = decode_video_frames
    video_utils_module.decode_video_frames = decode_video_frames
    lerobot_dataset_module._rlinf_pyav_decoder_installed = True


def _task_description_from_metadata(tasks: Any, task_index: int) -> Optional[str]:
    """Resolve a task string from old dict or LeRobot v3 DataFrame metadata."""
    task_index = int(task_index)
    if hasattr(tasks, "iloc"):
        columns = set(getattr(tasks, "columns", ()))
        if "task_index" in columns and "task" in columns:
        ...
        try:
            ...
            return None
        if hasattr(row, "index") and "task" in row.index:
            return str(row["task"])
        return None
...
class _LeRobotSource(TrajectorySource):
    ...
    def __init__(
        ...
        *,
        ...
        if not self._tasks:
            ...
            if hasattr(tasks, "iterrows") and {"task_index", "task"} <= set(
                tasks.columns
            ):
                self._tasks = {
            ...
            elif hasattr(tasks, "iterrows") and "task_index" in set(
                getattr(tasks, "columns", ())
            ):
                self._tasks = {
                    int(row["task_index"]): str(index)
    ...
    def _metadata_sample(self, episode: int, frame: int) -> dict:
        ...
        if task_idx is not None:
            task = _task_description_from_metadata(
                self.meta.tasks, int(_scalar_item(task_idx))
            )
            if task:
                sample["task"] = task
        return sample
